...
export function getRelatedPostsByTag(currentPostId, tag, limit = 3) {
  const allPosts = getSortedPostsData();
  ...
  // If we have 3 or fewer posts, just return them all
  if (taggedPosts.length <= limit) {
    return taggedPosts;
  }

  // Get the latest 2 articles (already sorted by date in getSortedPostsData)
  const latestTwo = taggedPosts.slice(0, 2);

  // Get one random article from the remaining posts
  const remainingPosts = taggedPosts.slice(2);
  const randomIndex = Math.floor(Math.random() * remainingPosts.length);
  const randomPost = remainingPosts[randomIndex];

  // Return the combination of latest two and one random
  return [...latestTwo, randomPost];
}
