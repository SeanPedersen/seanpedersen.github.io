...
Rust also shines through its ergnomic package manager cargo that provides a smooth developer UX. The only real downsides of Rust are its inherent complexity (slow compile speeds and huge compilation artifacts disk size - [solution?](https://github.com/jdx/mr-boxington), [deps gc](https://github.com/waynexia/cargo-gc)), still allowing unsafe code blocks and often verbose / ugly syntax.
