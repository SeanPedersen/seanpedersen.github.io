...
Rust also shines through its ergnomic package manager cargo that provides a smooth developer UX. The only real downsides of Rust are its inherent complexity (slow compile speeds and huge compilation artifacts disk size), still allowing unsafe code blocks and often verbose / ugly syntax.
